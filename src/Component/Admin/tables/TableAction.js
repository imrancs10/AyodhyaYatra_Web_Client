    ...
    return (
        <>
             <div className="table-actions d-flex align-items-center gap-3 fs-6">
                ...
                {option.showEdit && <div style={{ cursor: "pointer" }} onClick={e => option.edit.handler(dataId, data)} className="text-warning" data-bs-toggle={option?.edit?.showModel===true?"modal":""} data-bs-target={option?.edit?.showModel===true?"#" + (option.edit.modelId === undefined ? option.popupModelId : option.edit.modelId):""} title={option.edit?.title} data-toggle="tooltip" data-bs-placement="bottom" aria-label={option.edit?.title}><i className={option.edit.icon}></i></div>}
